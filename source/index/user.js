...
module.exports = (data) => {
  return new Promise((resolve) => {
    ...
    const ratio_tweets_day = data.statuses_count / age;

    // Similarity score
    let nameCut = name.replace(/[\s_]+/g, '');
    let screenNameCut = screenName.replace(/[\s_]+/g, '');
    ...
    else nameSimilarityScore = 1 - library.similarity(nameCut, screenNameCut); // calculates the similarity
    ...
    const totalSum = nameSimilarityScore + numberDigitScore + nameLengthScore + screenNameLengthScore + descriptionLengthScore + ageScore + ratioTweetScore + favoritesScore + imageScore;
